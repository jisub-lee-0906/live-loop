...
The locked model stack includes DiskCache 5.6.3. Advisory `GHSA-w8v5-vhqr-4h9v` has no fixed release listed in the 2026-09-23 OSV check. An attacker who can replace its local cache database may trigger unsafe pickle deserialization. No HTTP cache-database import/selection path was identified in this audit. Keep model/cache directories private, never load cache databases supplied by other users, and reassess this dependency when an upstream fix is available. This is a documented residual risk, not an all-dependencies-clear certification.
